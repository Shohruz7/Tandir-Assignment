...
export default function CartDrawer({ open, cart, onClose, onQty, onRemove, onClear, onCheckout }) {
  ...
  // send the order to the backend; onCheckout returns the saved order (with _id)
  async function checkout() {
    if (cart.length === 0) {
      setNotice({ type: "err", text: "Your cart is empty, add a dish or two first." });
      return;
    }
    setPlacing(true);
    setNotice(null);
    try {
      ...
      setNotice({
        type: "ok",
        text: `Rahmat! Order #${order._id.slice(-6)} placed for ${fmt(order.subtotal)}.`,
      });
    } catch (err) {
      setNotice({ type: "err", text: `Couldn't place order: ${err.message}` });
    } finally {
      setPlacing(false);
    }
  }
      ...
      >
        ...
        <div className="px-6 pt-5 pb-6 border-t border-white/10">
          {notice && (
            <div
              className={
                "mb-4 rounded-xl px-4 py-3 text-[0.9rem] " +
                (notice.type === "ok"
                  ? "bg-saffron/20 text-saffron"
                  : "bg-muted-red/20 text-[#f0b8ad]")
              }
            >
              {notice.text}
            </div>
          ...
          <div className="flex gap-3">
            ...
            <button
              onClick={checkout}
              disabled={placing}
              className="flex-1 rounded-full py-3 font-body font-bold bg-coral text-white hover:scale-[1.03] hover:brightness-110 transition-all disabled:opacity-60 disabled:cursor-not-allowed disabled:hover:scale-100"
            >
