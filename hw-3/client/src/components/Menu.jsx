...
export default function Menu({ onAdd }) {
  ...
  // pull the menu from the API once on mount
  useEffect(() => {
    let alive = true;
    getMenu()
      .then((data) => {
  ...
  const inTab = items.filter((i) => i.category === tab);
  ...
  return (
    <section id="menu" className="py-22 bg-cream">
      <div className="container-x">
        ...
        {/* states */}
        {status === "loading" && (
          <p className="text-center text-[#5a6b62] py-10">Loading the menu…</p>
        )}
        {status === "error" && (
          <p className="text-center text-muted-red py-10">
